#!/usr/bin/env python3
"""Generate sha256_compress_second_scalar.inc for Bitcoin double-SHA256 outer step."""
...
OUT_INC = SCRIPT_DIR.parent / "sha256_compress_second_scalar.inc"
...
def initial_slots() -> List[WSlot]:
    slots = [WSlot.var() for _ in range(8)]
    slots.append(WSlot.const(0x80000000))
    slots.extend(WSlot.const(0) for _ in range(6))
    slots.append(WSlot.const(0x00000100))
    return slots
...
def second_hash_reference(h: List[int]) -> List[int]:
    w = list(h) + [0x80000000, 0, 0, 0, 0, 0, 0, 0x00000100]
    return compress_reference_int(w, list(SHA256_IV))


def simulate_slots(h_words: List[int]) -> List[int]:
    w_int = list(h_words) + [0x80000000, 0, 0, 0, 0, 0, 0, 0x00000100]
    slots = initial_slots()
    a, b, c_, d, e, f, g, h = SHA256_IV
...
def generate_inc() -> str:
    lines: List[str] = [
        "// Auto-generated by tools/gen_sha256_second_compress.py — do not edit.",
        ...
        "",
        "void sha256_compress_second_16w(",
...
def gpu_selftest_first_digest() -> List[int]:
