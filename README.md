...
- **Dashboard — Pages 4–5 — Stratum wire JSON:** Two extra swipeable panels show the **last raw JSON line** received from the pool (**Stratum JSON Response**) and the **last raw JSON line** sent to the pool (**Miner JSON Submission**), refreshed while mining. When not mining, both show **Mining inactive**. Stored lines are **length-capped** in [`StratumClient.kt`](app/src/main/kotlin/com/btcminer/android/mining/StratumClient.kt) to limit RAM.
