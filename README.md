...
<img src="https://github.com/VirtusVerbis/BTC_Anrdoid_Miner/blob/main/Screenshot_1.jpeg" width="250" height="500"><img src="https://github.com/VirtusVerbis/BTC_Anrdoid_Miner/blob/main/Screenshot_2.jpeg" width="250" height="500">
...
- **Digital rain (Matrix-style background)** The main-screen animation adapts the column/trail logic from [Arduino DigitalRain Matrix](https://github.com/0015/Arduino_DigitalRain_Matrix) by Eric Nam ([`DigitalRainAnimation.hpp`](https://github.com/0015/Arduino_DigitalRain_Matrix/blob/main/src/DigitalRainAnimation.hpp)), **MIT License**. 
- **Config → Digital Rain**  exposes a master on/off switch, 
- **Canvas** vs **OpenGL ES backdrop** renderer choice: CPU, GPU 
- **glyph atlas** preset **MATRIX** (ASCII-style) vs **BITCOIN_VS_FIAT** (₿, ⚡, fiat/currency symbols), plus motion, color, character, optional **depth** streaks, and **key mode** highlights.
- The main activity can layer a **Satoshi Nakamoto** backdrop behind the rain, with an optional **Lightning** portrait variant.
- **CPU cores 0..N:** Configuration allows **0** through the device’s maximum CPU cores. **0** means no CPU worker threads; the CPU SHA-256 self-test runs only when cores > 0. With **GPU enabled**, mining can run **GPU-only**. If **CPU cores = 0** and **GPU is disabled**, **Start Mining** shows a toast (**Both CPU and GPU are disabled**) and does not start the service. See [`MiningConfig.hasActiveHashingConfig()`](app/src/main/kotlin/com/btcminer/android/config/MiningConfig.kt), [`MiningConstraints.canStartMining`](app/src/main/kotlin/com/btcminer/android/mining/MiningConstraints.kt), and [`NativeMiningEngine`](app/src/main/kotlin/com/btcminer/android/mining/NativeMiningEngine.kt) (CPU supervisor starts only when `threadCount > 0`; mining loop exits when CPU is off and GPU is not usable).
- **Nonce range split (CPU/GPU):** When **both** CPU and GPU mine, the 32-bit nonce space is split cleanly: CPU `0..7FFFFFFF`, GPU `80000000..FFFFFFFF` (no overlap). When **only one** backend is active, it searches the **full** nonce range (`0..FFFFFFFF`). Policy lives in [`NonceRangePolicy.kt`](app/src/main/kotlin/com/btcminer/android/mining/NonceRangePolicy.kt); on mining start, logcat tag **Mining** logs `Nonce ranges: CPU=... GPU=...` for on-device confirmation.
...
- **Sudden app death during long GPU mining sessions:** Some runs could terminate unexpectedly when repeated per-round GPU worker thread churn increased native memory pressure and triggered LMK kills. **Fix:** [`NativeMiningEngine`](app/src/main/kotlin/com/btcminer/android/mining/NativeMiningEngine.kt) now reuses a single long-lived GPU worker executor thread instead of creating a new GPU worker thread each round. This reduces memory growth and improves long-run stability under sustained GPU mining.
...
- **Mining charts (below the five-page dashboard):** A second pager + dot row in [`activity_main.xml`](app/src/main/res/layout/activity_main.xml), wired by [`ChartPagerAdapter.kt`](app/src/main/kotlin/com/btcminer/android/ChartPagerAdapter.kt) ([`MainActivity.kt`](app/src/main/kotlin/com/btcminer/android/MainActivity.kt)). **Tab 1 — Hash rate:** [`ChartHashrateFragment`](app/src/main/kotlin/com/btcminer/android/ChartHashrateFragment.kt); tap the chart to cycle **2 min** / **10 min** / **session** history. **Tab 2 — Shares:** [`ChartSharesDonutFragment`](app/src/main/kotlin/com/btcminer/android/ChartSharesDonutFragment.kt) — CPU vs GPU **identified** shares donut. **Tab 3 — Best difficulty:** [`ChartBestDifficultyFragment`](app/src/main/kotlin/com/btcminer/android/ChartBestDifficultyFragment.kt); tap the title for **History** (event order, gaps not shown as wall-clock time) vs **this session** (ordered improvements). **Tab 4 — Fractal art:** [`ChartMandelbrotFragment`](app/src/main/kotlin/com/btcminer/android/ChartMandelbrotFragment.kt); images come from **session best difficulty** snapshots rendered by [`MandelbrotEscapeRenderer.kt`](app/src/main/kotlin/com/btcminer/android/util/MandelbrotEscapeRenderer.kt). Tap the fractal image to cycle [`FractalPlotKind`](app/src/main/kotlin/com/btcminer/android/util/FractalPlotKind.kt) (Mandelbrot, Julia, Burning Ship, Multibrot, Tricorn, Phoenix, Newton). In-app copy: [`strings.xml`](app/src/main/res/values/strings.xml) (`mandelbrot_chart_placeholder`, `fractal_chart_caption_pattern`).
- **Chart axes and value labels:** Hash rate and best-difficulty MPAndroidChart panels use tightened axis formatting and layout in [`MainActivity.kt`](app/src/main/kotlin/com/btcminer/android/MainActivity.kt).
- **Digital Rain (recent stack):** OpenGL rain startup failure falls back to Canvas (see `digital_rain_gpu_fallback` in [`strings.xml`](app/src/main/res/values/strings.xml)); GPU glyph atlases cache ₿/⚡/fiat pools in [`DigitalRainGlRenderer.kt`](app/src/main/kotlin/com/btcminer/android/ui/digitalrain/gl/DigitalRainGlRenderer.kt) / [`RainGlyphAtlas.kt`](app/src/main/kotlin/com/btcminer/android/ui/digitalrain/gl/RainGlyphAtlas.kt). Session chart snapshots and best-difficulty events that feed the fractal tab are coordinated from [`MainActivity.kt`](app/src/main/kotlin/com/btcminer/android/MainActivity.kt) (e.g. `MandelbrotRenderSnapshot`, `BestDifficultyChartEvent`).
...
- **CPU and GPU** mining: configurable **CPU cores (0..device maximum; 0 = CPU off)** and **CPU intensity (1–100%)**, **GPU enabled** switch, **GPU local_size_x (threads per workgroup, 32..device max, step 32)**, **GPU hash per thread (1/2/4/8, default 1)**, and **GPU intensity (0–100%)**. **GPU uses Vulkan compute (SPIR-V) only**—no CPU fallback; if the GPU path is unavailable, the dashboard shows "----" for GPU hashrate and a one-time toast "GPU not available". Build requires Vulkan SDK (or glslc) so the shader compiles to SPIR-V.
- **GPU local_size_x:** user-tuned threads per workgroup; `vkCmdDispatch` group count is computed automatically from chunk size and local_size_x. **GPU hash per thread:** scalar hashes each thread runs per invocation (1, 2, 4, or 8). Dashboard label shows "Hash Rate (GPU) - {local_size_x}×{hash_per_thread}" when GPU is enabled.
